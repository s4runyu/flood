...
class Model {
    ...
    Model(Model model) {
        ...
        _cells = new int[model.height()][model.width()];
    ...
    void forNeighbors(Place place, Consumer<Place> action) {
        int rowOfPlace = place.row;
        int colOfPlace = place.col;
        boolean isTopValid = isCell(rowOfPlace - 1, colOfPlace);
        boolean isRightValid = isCell(rowOfPlace, colOfPlace + 1);
        boolean isBottomValid = isCell(rowOfPlace + 1, colOfPlace);
        boolean isLeftValid = isCell(rowOfPlace, colOfPlace - 1);
        if (isTopValid) {
            action.accept(Place.pl(rowOfPlace - 1, colOfPlace));
        }
        if (isRightValid) {
            action.accept(Place.pl(rowOfPlace, colOfPlace + 1));
        }
        if (isBottomValid) {
            action.accept(Place.pl(rowOfPlace + 1, colOfPlace));
        }
    ...
    boolean[] colorsPresent() {
        boolean[] colors = new boolean[this.ncolors()];
        ArrayList<Integer> availableColors = new ArrayList<Integer>();
        for (int i = 0; i < this.height(); i++) {
            for (int j = 0; j < this.width(); j++) {
                if (!availableColors.contains(this.get(i, j))) {
                    availableColors.add(this.get(i, j));
                }
            }
        }
        for (int i = 0; i < colors.length; i++) {
            colors[i] = availableColors.contains(i) ? true : false;
        }
        return colors;
    }
    ...
    boolean roundOver() {
        return this.solved() || this.numMoves() == this._limit;
    }
    ...
    HashSet<Place> findRegion(Place start, int color, HashSet<Place> result) {
        if (!result.contains(start) && _cells[start.row][start.col] == color) {
            result.add(start);
        }
        boolean isTopValid = isCell(start.row - 1, start.col);
        boolean isRightValid = isCell(start.row, start.col + 1);
        boolean isBottomValid = isCell(start.row + 1, start.col);
        boolean isLeftValid = isCell(start.row, start.col - 1);
        if ((!isTopValid || (_cells[start.row - 1][start.col] != color
                || result.contains(pl(start.row - 1, start.col))))
                && ((!isRightValid || (_cells[start.row][start.col + 1] != color
                || result.contains(pl(start.row, start.col + 1)))))
                && (!isBottomValid || (_cells[start.row + 1][start.col] != color
                || result.contains(pl(start.row + 1, start.col))))
                && (!isLeftValid || (_cells[start.row][start.col - 1] != color
                || result.contains(pl(start.row, start.col - 1))))) {
            return result;
        } else {
            this.forNeighbors(start, (p) -> {
                if (!result.contains(p) && _cells[p.row][p.col] == color) {
                    result.add(p);
    ...
    void setActiveRegionColor(int newColor) {
        if (newColor != _cells[0][0]) {
            for (Place activePlace : _active) {
                _cells[activePlace.row][activePlace.col] = newColor;
            }
            _active = findRegion(pl(0, 0), _cells[0][0], new HashSet<>());
            GameState gs = new GameState();
            gs.saveState();
            _history.add(_current + 1, gs);
            _current += 1;
            _lastHistory += 1;
            clearMarks();
        }
    ...
    Set<Place> adjacentCells(int color) {
        HashSet<Place> currentActiveCells = sameColorRegion(pl(0, 0));
        this.setActiveRegionColor(color);
        HashSet<Place> adjacentActiveCells = sameColorRegion(pl(0, 0));
        this.undo();
        HashSet<Place> result = new HashSet<Place>();
        for (Place p: adjacentActiveCells) {
            if (!currentActiveCells.contains(p)) {
                result.add(p);
            }
        }
        return result;
