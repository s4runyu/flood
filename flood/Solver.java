...
class Solver {
    ...
    static int[] chooser(Model model, int depth) {
        ...
        boolean[] colors = model.colorsPresent();
        int bestMove = 0;
        int maxSize = 0;
        int dummyBestMove = 0;
        int dummyMaxSize = 0;
        for (int i = 0; i < model.ncolors(); i++) {
            if (colors[i] && i != model.get(0, 0)) {
                dummyBestMove = i;
            }
        }
        bestMove = dummyBestMove;
        maxSize = dummyMaxSize;
        for (int i = 0; i < model.ncolors(); i++) {
            Model copyModel = new Model(model);
            if (colors[i] && i != model.get(0, 0)) {
                copyModel.setActiveRegionColor(i);
                int currentSize = copyModel.activeRegionSize();
